...
const Chat = () => {
  ...
  return (
    <>
      ...
      {/* СЕКЦИЯ ЧАТА */}
      {isOpen && (
        <section id="chat-ai" className="py-12 bg-white border-t border-gray-50">
          <div className="max-w-3xl mx-auto px-4">
            <div className="text-center mb-6">
              <h2 className="text-3xl font-black text-gray-900">Sino AI</h2>
              <p className="text-sm text-gray-400 mt-1">{lang === 'ru' ? 'Ваш medical помощник' : 'Sizning tibbiy yordamchingiz'}</p>
            </div>
            ...
            <div className="flex flex-col h-[550px] bg-white rounded-2xl border border-gray-100 shadow-[0_15px_40px_rgba(16,185,129,0.08)] overflow-hidden">
              ...
              <div ref={scrollRef} className="flex-1 p-6 space-y-6 overflow-y-auto bg-white">
                {messages.length === 0 && (
                  <div className="h-full flex items-center justify-center text-gray-300">
                    <p className="text-lg font-medium">{lang === 'ru' ? 'Начните диалог' : 'Muloqotni boshlang'}</p>
                  </div>
                )}
              ...
              <form onSubmit={sendMessage} className="p-4 bg-white border-t border-gray-100 flex gap-3 items-center">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 rotate-90" viewBox="0 0 20 20" fill="currentColor">
