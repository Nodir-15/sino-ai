          ...
          <div className="flex border border-gray-200 rounded-full overflow-hidden bg-white h-8">
            {['uz', 'ru', 'en'].map(l => (
              <button key={l} onClick={() => i18n?.changeLanguage(l)}
                className={`px-3 text-[11px] font-bold ${currentLang === l ? 'bg-[#00A36C] text-white' : 'text-gray-400 hover:bg-gray-50'}`}>
                {l.toUpperCase()}
              </button>
            ))}
          </div>
          <button className="bg-[#0D1B15] text-white px-5 py-2 rounded-full text-sm font-bold shadow-lg hover:bg-black transition-all active:scale-95">
            {t.partnerBtn}
          </button>
        </div>
      </div>
