...
const BusinessNavbar = () => {
  ...
  return (
    <nav className="sticky top-0 z-[100] w-full bg-white/95 backdrop-blur-md border-b border-gray-100 py-4 font-sans text-black">
        ...
        <div className="flex items-center gap-4">
          <div className="flex border border-gray-200 rounded-full overflow-hidden bg-white h-8 shadow-sm">
            {['uz', 'ru', 'en'].map(l => (
              <button 
                key={l} 
                onClick={() => i18n?.changeLanguage(l)} // МЕНЯЕМ ЯЗЫК
