        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {team.map((m, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.1 }}
              ...
              <ul className="space-y-1">{m.p.map((pt, idx) => <li key={idx} className="text-[12px] text-gray-400 flex items-start gap-2 font-sans"><span className="text-[#3E9E67]">•</span> {pt}</li>)}</ul>
            </motion.div>
          ))}
        </div>
      </div>
